            ...
            for batch_num, batch in enumerate(clustered_batches):
                for dot_index, dot in enumerate(batch):
                    if dot[0] == 0:  # Check if the flag is 0
                        ...
                        if input_string == "2.":
                            if dot_index in [0, 1]:
                                output_content += f"{input_string}{gap_string+1}.{chr(ord('a') + batch_num)} {x},{y},{width},{height} "
                            else:
                                output_content += f"{input_string}{gap_string+2}.{chr(ord('a') + batch_num)} {x},{y},{width},{height} "                            
